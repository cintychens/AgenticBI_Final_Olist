...
- `outputs/` 为运行时生成目录，通常不建议提交到 GitHub。
- `data/` 中的原始 CSV 文件较大，通常不建议提交到 GitHub。
- 如果使用公开仓库，请移除真实数据库密码和 API Key。
- MySQL 普通 View 不一定总能带来显著加速；如需展示更明显的性能提升，可在报告中说明普通视图与物理缓存表的区别。
